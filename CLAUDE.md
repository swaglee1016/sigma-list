...
Sigma List — 艾森豪威尔矩阵待办应用，Vite + Capacitor 跨平台（Web / Android / iOS）。米色海报风，全站 UI 英文，纯本地存储。
...
- **数据层**：`tasks` 对象 `{ un, ue, nn, ne }` 按四象限分区，`notes` 数组。通过 `@capacitor/preferences`（原生）或 `localStorage`（浏览器）持久化。
- **Task 模型**：`{ id, text, tips, done, ts, updatedAt, dueDate, reminderEnabled, reminderTime }`。旧数据自动迁移补全。
- **Note 模型**：`{ id, title, body, ts }`
...
**刷新链**：数据变更 → `saveTasks()`/`saveNotes()` → `refreshAll()` → 各视图 render 函数。
...
src/
  ...
  services/              # notifications / haptics / export
